console.log("🎉 Coupon validation script loaded successfully!");
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function validateDiscount(valueInput, typeSelect, maxDiscountInput) {
        if (!valueInput || !typeSelect) return false; 
    ...
    function validateMinOrderAmount(input, discountValueInput, discountTypeSelect) {
        if (!input || !discountValueInput || !discountTypeSelect) return false; 
    ...
    function validateUsageLimits(globalInput, perUserInput) {
        if (!globalInput || !perUserInput) return false; 
    ...
    function validateCouponForm(form, isEdit = false) {
        ...
        if (codeInput) isValid = validateCode(codeInput) && isValid;
        if (descInput) isValid = validateDescription(descInput) && isValid;
        if (discountValueInput && discountTypeSelect) isValid = validateDiscount(discountValueInput, discountTypeSelect, maxDiscountInput) && isValid;
        if (minOrderInput && discountValueInput && discountTypeSelect) isValid = validateMinOrderAmount(minOrderInput, discountValueInput, discountTypeSelect) && isValid;
        if (startDateInput && endDateInput) isValid = validateDates(startDateInput, endDateInput, isEdit) && isValid;
        if (globalLimitInput && perUserLimitInput) isValid = validateUsageLimits(globalLimitInput, perUserLimitInput) && isValid;
    ...
    // Add real-time validation event listeners with null checks
        ...
        ['startDate', 'editStartDate', 'expiryDate', 'editExpiryDate'].forEach(id => {
            ...
            if (input) {
                input.addEventListener('change', function() {
                    ...
                    const endInput = document.getElementById(prefix + 'ExpiryDate');
